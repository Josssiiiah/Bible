...
export class AuthServer {
  ...
  private getKeysFilePath(): string {
    // Handle both development and compiled Mastra paths
    const serverRootPath = process.cwd().includes(".mastra")
      ? path.join(process.cwd(), "../../") // Go up from .mastra/output to server root
      : process.cwd(); // Already at server root

    return path.join(serverRootPath, "gcp-oauth.keys.json");
  }
  ...
  private async loadCredentials(): Promise<void> {
    try {
      ...
      throw new Error(
        "Failed to load OAuth credentials. Make sure gcp-oauth.keys.json is in the root directory."
      );
    }
  }
      ...
      try {
        ...
        this.server.get("/oauth2callback", async (req, res) => {
          try {
            ...
            // Allow the response to be sent before stopping the server
...
// Helper function to initialize the OAuth client and start the auth flow
export async function initializeGoogleAuth(): Promise<OAuth2Client | null> {
  try {
    // Handle both development and compiled Mastra paths
    const serverRootPath = process.cwd().includes(".mastra")
      ? path.join(process.cwd(), "../../") // Go up from .mastra/output to server root
    ...
    const keysPath = path.join(serverRootPath, "gcp-oauth.keys.json");
    ...
    const hasValidTokens = await tokenManager.loadSavedTokens();

    // If no valid tokens, start the auth flow
    if (!hasValidTokens) {
